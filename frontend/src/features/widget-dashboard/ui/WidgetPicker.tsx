...
import { WIDGET_REGISTRY, WIDGET_TEMPLATES } from '../lib/widget-registry';
import type { WidgetType, WidgetTemplate } from '../types/widget.types';
...
const ICON_SVG: Record<string, React.ReactNode> = {
  Gauge: (
    <path
      ...
      d="M12 3v1m0 16v1M4.22 4.22l.707.707m12.02 12.02.708.708M3 12h1m16 0h1M4.927 19.073l.707-.707M18.364 5.636l.707-.707M12 8a4 4 0 1 1 0 8 4 4 0 0 1 0-8Z"
    />
  ),
  Wifi: (
    <path
  ...
  ToggleRight: (
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M5.25 14.25h13.5m-13.5 0a3 3 0 0 1-3-3m3 3a3 3 0 1 0 6 0m-6 0H5.25m8.25 0a3 3 0 0 0 3-3m-3 3a3 3 0 1 1-6 0m6 0h2.25M12 11.25a3 3 0 1 1 0-6 3 3 0 0 1 0 6Z"
    />
  ),
  Zap: (
    <path
  ...
  GaugeDial: (
    <>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M3.75 12a8.25 8.25 0 1 1 16.5 0"
      />
      <path strokeLinecap="round" strokeLinejoin="round" d="m12 12 4-3" />
    </>
  ),
  CircleProgress: (
    <>
      <circle cx="12" cy="12" r="9" strokeWidth={1.5} />
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 3a9 9 0 0 1 6.36 15.36" />
    </>
  ),
  Sliders: (
    <path
      strokeLinecap="round"
      strokeLinejoin="round"
      d="M10.5 6h10.5M3 6h4.5m3 6h10.5M3 12h4.5m3 6h10.5M3 18h4.5M9 4.5v3M9 10.5v3M9 16.5v3M15 7.5v.001"
    />
  ),
  CardHero: (
    <>
...
function IconBox({ icon, className }: { icon: string; className?: string }) {
  return (
    <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${className ?? 'bg-primary/10'}`}>
      <svg
        ...
        <div className="overflow-y-auto p-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
          {tab === 'templates'
            ...
            : WIDGET_REGISTRY.map((meta) => (
                ...
                >
                  <IconBox icon={meta.icon} className="bg-primary/10 text-primary" />
                  <div className="min-w-0">
