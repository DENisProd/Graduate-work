...
export function normalizeZigbeePayload(
  payload: Record<string, unknown>,
): ZigbeeNormalized {
  const out: ZigbeeNormalized = {};

  const state = payload.state;
  if (typeof state === 'string') out.state = state;

  const brightness = asNumber(payload.brightness);
  if (brightness !== undefined) out.brightness = Math.trunc(brightness);

  const linkquality = asNumber(payload.linkquality);
  if (linkquality !== undefined) out.linkquality = Math.trunc(linkquality);

  const colorMode = payload.color_mode ?? payload.colorMode;
  if (typeof colorMode === 'string') out.colorMode = colorMode;
