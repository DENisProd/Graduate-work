import { X, Radio, CheckCircle, AlertCircle, Loader2, LogOut } from 'lucide-react'
...
const EVENT_ICONS: Record<PairingEvent['type'], React.ReactNode> = {
  ...
  device_leave: <LogOut className="h-4 w-4 text-slate-400" />,
  interview_started: <Loader2 className="h-4 w-4 animate-spin text-slate-400" />,
  interview_successful: <CheckCircle className="h-4 w-4 text-emerald-600" />,
  interview_failed: <AlertCircle className="h-4 w-4 text-red-500" />,
}

const EVENT_LABELS: Partial<Record<PairingEvent['type'], string>> = {
...
export function PairingDialog({ open, onClose }: PairingDialogProps) {
  ...
  const pairedDevices = events.filter((e) => e.type === 'interview_successful')
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        {t('common.secondsLeft', { count: seconds })}
                      </p>
                    </div>
                  </div>
                  <button
            ...
            {pairedDevices.length > 0 && (
              <div className="space-y-1.5">
                <p className="text-xs font-medium uppercase tracking-wide text-emerald-600 dark:text-emerald-400">
                  {t('pairing.pairedDevices')} ({pairedDevices.length})
                </p>
                <div className="space-y-1.5">
                  {pairedDevices.map((e, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 dark:border-emerald-800 dark:bg-emerald-900/20"
                    >
                      <CheckCircle className="h-5 w-5 shrink-0 text-emerald-500" />
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-emerald-800 dark:text-emerald-200">
                          {e.friendlyName ?? e.ieeeAddr}
                        </p>
                        {e.model && (
                          <p className="text-xs text-emerald-600 dark:text-emerald-400">{e.model}</p>
                        )}
                      </div>
                    </div>
                  ))}
                  ...
                  {events.map((event, i) => (
                    <div
                      ...
                      className={cn(
                        ...
                        event.type === 'interview_successful' && 'bg-emerald-50 dark:bg-emerald-900/10',
                        event.type === 'interview_failed' && 'bg-red-50 dark:bg-red-900/10',
                        event.type === 'device_leave' && 'opacity-60',
                      )}
                    >
                      <span className="mt-0.5 shrink-0">
                        {EVENT_ICONS[event.type] ?? <Radio className="h-4 w-4 text-slate-400" />}
                      </span>
                      <div className="min-w-0">
