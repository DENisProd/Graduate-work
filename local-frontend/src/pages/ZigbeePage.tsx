  ...
  return (
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-80 -translate-x-1/2 -translate-y-1/2 rounded-xl bg-white p-6 shadow-2xl dark:bg-slate-900">
        <h3 className="font-semibold text-slate-900 dark:text-slate-100">Delete device?</h3>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
          Remove <span className="font-mono font-medium">{device.friendlyName ?? device.ieeeAddr}</span> from Zigbee network.
...
export function ZigbeePage() {
  ...
  const syncMutation = useMutation({
    ...
    onSuccess: (result) => {
      toast.success(`Synced ${result.count ?? 'all'} devices`)
        ...
        <>
          {isError && (
            <div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-400">
              Failed to load Zigbee devices. Check that the local server is running.
            </div>
          )}
          ...
          {isPending ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="font-medium text-slate-700 dark:text-slate-300">No Zigbee devices</p>
              <p className="mt-1 text-sm text-slate-400">Click "Sync from Bridge" to fetch devices</p>
            </div>
          ) : (
            <>
